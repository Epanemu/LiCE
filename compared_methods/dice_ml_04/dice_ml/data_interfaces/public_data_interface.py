...
class PublicData:
    ...
    def __init__(self, params):
        ...
        if "data_handler" in params:
            self.dhandler = params["data_handler"]
            self.encoded_feature_names = params["one_hot_cols"]
            self.one_hot_encoded_data = self.encode(self.data_df)
        else:
            raise ValueError("No data handler, cannot run.")
    ...
    def encode(self, data):
        ...
        if self.outcome_name in data.columns:
            res = np.concatenate([self.dhandler.encode(data.drop(columns=[self.outcome_name])),
                                  self.dhandler.encode_y(data[self.outcome_name]).reshape(-1,1)],
                                  axis=1)
            return pd.DataFrame(res, columns=self.encoded_feature_names + [self.outcome_name])
        return pd.DataFrame(self.dhandler.encode(data), columns=self.encoded_feature_names)

    def decode(self, data):
        return self.dhandler.decode(data.values)
    ...
    def get_decimal_precisions(self):
        ...
        for ix, col in enumerate(self.encoded_feature_names):
            if col not in self.continuous_feature_names:
                continue
            ...
            elif((self.encode(self.data_df)[col].dtype == np.float32) or (self.encode(self.data_df)[col].dtype == np.float64)):
                modes = self.encode(self.data_df)[col].mode()
